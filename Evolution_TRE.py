...
st.markdown('''
  <p class='title' style="font-weight: 700; font-size: 3rem; line-height: 100%;">Evolution du Taux de Rémunération de <br>l'Epargne depuis 1988 🇹🇳</p>  
...
df = pd.read_excel('BCT TRE.xlsx', sheet_name='Feuil1')
...
def df_style(value):
    if value == principal_max_rate:
        ...
        return 'background-color: #FFE8C5;'
    else:
        return ''
...
if Options == None:
    st.markdown(f'<p style="font-size: .8em; color: gray;">*Période: 1988 à {now.year}</p>',unsafe_allow_html=True)
else:
...
with st.container():
    ...
    if Options != None :
        ...
        with col2:
            with st.container(border=True):
                fig = px.line(df_filtred, x='Date', y='Taux (%)', title='Variation du TRE par année et par mois', line_shape='spline', color_discrete_sequence=['#FF0000'])
                fig.add_hline(y=mean_value, line_dash='dash', line_color='#FFA27F', annotation_text=f'Ligne de Moyenne : {mean_value}', annotation_position='bottom left')
        ...
        with col2:
            with st.container(border=True):
                fig = px.line(df, x='Date', y='Taux (%)', title='Variation du TRE par année et par mois', line_shape='spline', color_discrete_sequence=['#FF0000'])
                fig.add_hline(y=mean_value, line_dash='dash', line_color='#FFA27F', annotation_text=f'Ligne de Moyenne : {mean_value}', annotation_position='bottom left')
                st.plotly_chart(fig, use_container_width=True)
...
if actual_rate_growth >= 0 :
    st.markdown(f''' 
<p>Le taux de rémunération de l'épargne TRE du mois de {month_in_french} {now.year} s'élève à <span style="font-family: Source Code Pro; font-size: .8em; color: rgb(9, 171, 59); background: rgb(248, 249, 251);padding: .2em .4em; border-radius: .25rem">{actual_month_rate}%</span>, avec une évolution de <span style="font-family: Source Code Pro; font-size: .8em; color: rgb(9, 171, 59); background: rgb(248, 249, 251);padding: .2em .4em; border-radius: .25rem">▴{actual_rate_growth.round(2)}%</span> par rapport au mois précédent.</p>
''', unsafe_allow_html=True)
else:
    st.markdown(f''' 
<p>Le taux de rémunération de l'épargne TRE du mois de {now.strftime('%B').encode('latin1').decode('utf-8')} {now.year} s'élève à <span style="font-family: Source Code Pro; font-size: .8em; color: rgb(9, 171, 59); background: rgb(248, 249, 251);padding: .2em .4em; border-radius: .25rem">{actual_month_rate}%</span>, avec une évolution de <span style="font-family: Source Code Pro; font-size: .8em; color: red; background: rgb(248, 249, 251);padding: .2em .4em; border-radius: .25rem">▾{actual_rate_growth.round(2)}%</span> par rapport au mois précédent.</p>
''', unsafe_allow_html=True)

if Options == None:
    st.markdown(f'''
<p>Le taux moyen de la période s'étalant de 1988 à {now.year} s'élève à <span style="font-family: Source Code Pro; font-size: .8em; color: rgb(9, 171, 59); background: rgb(248, 249, 251);padding: .2em .4em; border-radius: .25rem">{avg_rate}%</span>.
